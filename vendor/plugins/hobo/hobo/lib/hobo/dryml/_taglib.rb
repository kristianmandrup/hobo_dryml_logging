  ...
  module Dryml
    class Taglib
      ...
      class << self
        ...
        def get(options)
          log "Hobo::Dryml::Taglib.get(options) : BEGIN"                   
          ...
          log "Hobo::Dryml::Taglib.get(options) : END"                             
          taglib
        end
        ...
        def taglib_filename(options)
          ...
          log "Hobo::Dryml::Taglib.taglib_filename(options) #{str}"         
          base = if plugin == "hobo"
                   "#{HOBO_ROOT}/taglibs"
                 elsif plugin
          ...
          log "Hobo::Dryml::Taglib.taglib_filename(options) - filename: #{filename} : END"
          filename
        end
      ...
      def load  
        log "Hobo::Dryml::Taglib.load module : BEGIN"              
        ...
        log "Hobo::Dryml::Taglib - new template #{@src_file} : BEGIN"                              
        template = Template.new(File.read(@src_file), @module, @src_file, @bundle)
        log "Hobo::Dryml::Taglib - new template #{@src_file} : END"                              

        log "Hobo::Dryml::Taglib - template compile : BEGIN"                                      
        template.compile([], [])
        log "Hobo::Dryml::Taglib - template compile : END"                                      
        
        # template.set_context(@context) 
        log "Hobo::Dryml::Taglib.load module : END"                      
