module Hobo::Dryml
  ...
  class TemplateEnvironment
    ...
    def call_polymorphic_tag(name, *args)      
      log "TemplateEnvironment::call_polymorphic_tag: <#{name}> : BEGIN"      
      ...
      log "TemplateEnvironment::call_polymorphic_tag: <#{name}> : END"            
      res
    end


    def find_polymorphic_tag(name, call_type=nil)
      log "TemplateEnvironment::find_polymorphic_tag: <#{name}> : BEGIN"      
      ...
      log "TemplateEnvironment::find_polymorphic_tag: </#{name}> : END"            
    end
    ...
    def call_tag_parameter_with_default_content(the_tag, attributes, default_content, overriding_content_proc)
      log "TemplateEnvironment::call_tag_parameter_with_default_content : BEGIN"
      
      separator = attributes.key?(:separator) ? attributes[:separator] : ""       
      
      tres = if the_tag.is_one_of?(String, Symbol) && the_tag.to_s.in?(Hobo.static_tags)
        log "<#{the_tag}>"
        ...
        # push separator on env_context stack
        # tag_name = "my_tag"
        # if the_tag.is_one_of?(String, Symbol)
        #   tag_name =  the_tag.to_s
        # end 
        # log " tag_name: #{tag_name}"
        # @context << {:tag => tag_name, :separator => true}
        log "SEPARATOR: #{separator} for tag <#{the_tag}>"
        log "send(#{the_tag}, #{attributes.inspect}, params with :separator)"
        out = send(the_tag, attributes, { :default => d, :separator =>  separator})                                
        out
      end
      log "TemplateEnvironment::call_tag_parameter_with_default_content : END"
      tres      
    end
    ...
    def call_tag_parameter(the_tag, attributes, parameters, caller_parameters, param_name)
      ...
      log "call_tag_parameter - <#{the_tag}> : BEGIN"      
      ...
      if param_name == :default && overriding_proc
        ...
        res = call_tag_parameter_with_default_content(the_tag, attributes, parameters[:default], overriding_proc)
        ...
        res = replacing_proc.call(tag_restore)

      else
        overriding_attributes, overriding_parameters = overriding_proc._?.call
        res = override_and_call_tag(the_tag, attributes, parameters, overriding_attributes, overriding_parameters)        
      end
      log "call_tag_parameter - </#{the_tag}> : END"      
      res
    end


    def override_and_call_tag(the_tag, general_attributes, general_parameters, overriding_attributes, overriding_parameters)
      log "TemplateEnvironment::override_and_call_tag : BEGIN"
      ...
      tres = if the_tag.is_one_of?(String, Symbol) && the_tag.to_s.in?(Hobo.static_tags)
        log "A static Hobo tag: <#{the_tag}> BEGIN"
        body = if overriding_default_content
                 log "body: call overriding_default_content"          
                 new_context { overriding_default_content.call(proc { default_content.call(nil) if default_content }) }
               elsif default_content
                 log "body: call default_content"
                 new_context { default_content.call(nil) }
               else
                 log "body: nil"
                 nil
               end            
        res = element(the_tag, attributes, body)
        log "A static Hobo tag: </#{the_tag}> : END"        
        res
      else
        if default_content || overriding_default_content
          log "has default content: <#{the_tag}>"
        ...
        if the_tag.is_one_of?(String, Symbol)
          ...
          log "A defined DRYML tag: <#{the_tag}> : BEGIN"
          log "attributes:" + attributes.inspect
          # log parameters.inspect
          res = send(the_tag, attributes, parameters)
          log "A defined DRYML tag: </#{the_tag}> : END"          
          res
        else
          # It's a proc - restoring a replaced parameter
          log "Calling proc : BEGIN"          
          log "attributes:" + attributes.inspect
          # log parameters.inspect
          res = the_tag.call(attributes, parameters)
          log "Calling proc : END"                    
          res
        end        
      end
      log "TemplateEnvironment::override_and_call_tag : END"      
      tres
    end
    ...
    def merge_tag_parameter(general_proc, overriding_proc)
      log "TemplateEnvironment::merge_tag_parameter : BEGIN"      
      if overriding_proc.nil?
        res = general_proc
      else
        res = if overriding_proc.arity == 1
      ...
      log "TemplateEnvironment::merge_tag_parameter : END"            
      res
    end
    ...
    def render_tag(tag_name, attributes)
      log "TemplateEnvironment:: render_tag <#{tag_name}> : BEGIN"      
      method_name = tag_name.to_s.gsub('-', '_')
      tres = if respond_to?(method_name)
      ...
      log "TemplateEnvironment:: render_tag </#{tag_name}> : END"            
      tres
    end


    def element(name, attributes, content=nil, escape = true, empty = false, &block)
      log "TemplateEnvironment:: element <#{name}> : BEGIN"
      ...
      tres = if block_called_from_erb? block
        concat res
      else
        res
      end
      log "TemplateEnvironment:: element </#{name}> : END"      
      tres
    end
